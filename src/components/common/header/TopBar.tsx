...
import {
  selectFavsItemsList,
  selectFavsTotalQty,
} from "@/redux/features/favs/favsSelectors";
import {
  selectCartItemsList,
  selectCartTotalQty,
...
const TopBar = ({ disabled, className }: Props) => {
  ...
  const handleProfileLoginClick = (text: string): void => {
    text === "Profile" && setIsLoginOpen((prev) => !prev);

    text === "Cart" && cartOpen();

    text === "Wishlist" && favsOpen();

    text === "Compare" && compareOpen();
  };
            ...
            >
              <span className="text-[20px] group-hover:scale-110 transition relative">
                ...
                {data.text === "Compare" && (
                  <div className="absolute -top-2.5 -right-2.5">
                    <div className="w-5 h-5 bg-primary/85 text-white txt-tiny grid place-items-center rounded-full">
                      {cartItemsList.length + favsItemsList.length}
                    </div>
                  </div>
                )}
              </span>
              <p className="max-sm:hidden txt-tiny">{data.text}</p>
            </button>
          ))}
